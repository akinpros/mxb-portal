...
import type { Producer, PipelineFilm, Announcement } from '@/lib/types'
...
type Tab = 'home' | 'myfilms' | 'pipeline' | 'messages'

type PMsg = { id: string; sender: string; body: string; read_by_admin: boolean; created_at: string }
...
export default function ProducersClient({ user, producer, pipeline, announcements, messages: initMessages }: Props) {
  ...
  async function sendMessage(e: React.FormEvent) {
    e.preventDefault()
    if (!producer || !msgBody.trim()) return
    setSending(true)
    const { data } = await supabase
      .from('mxb_producer_messages')
      .insert({ producer_id: producer.id, sender: 'producer', body: msgBody })
      .select().single()
    if (data) setMessages(prev => [data as PMsg, ...prev])
    setMsgBody('')
    setMsgSent(true)
    setSending(false)
    ...
    <div style={{ minHeight: '100vh', background: BG, fontFamily: 'Georgia,serif', color: '#fff' }}>
      ...
      <nav style={{
        ...
        <div style={{ display: 'flex', gap: 14, alignItems: 'center', flexWrap: 'wrap' }}>
          {navBtn('home', 'Inicio')}
          {navBtn('myfilms', 'Mis Proyectos')}
          {navBtn('pipeline', 'Pipeline MXB')}
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '38px 22px 80px' }}>
        ...
        {tab === 'home' && (
          <div>
            {/* Hero */}
            <div style={{ borderRadius: 12, overflow: 'hidden', marginBottom: 32, background: 'linear-gradient(135deg,#0d0d0d,#1a1208)', border: `1px solid ${G}` }}>
              <div style={{ padding: '54px 42px' }}>
                <div style={{ fontSize: 10, letterSpacing: '.32em', textTransform: 'uppercase', color: CH, marginBottom: 10 }}>
                  Productor · The Wonder World Group
                </div>
                <h2 style={{ fontWeight: 300, fontSize: 36, color: G, marginBottom: 8 }}>
                  Bienvenido, {producer?.contact_name ?? user.email}
                </h2>
                {producer?.company_name && (
                  <p style={{ color: CH, fontSize: 15, letterSpacing: '.1em', marginBottom: 16 }}>{producer.company_name}</p>
                )}
                <div style={{ display: 'inline-block', border: `1px solid ${G}`, color: G, fontSize: 10, letterSpacing: '.3em', textTransform: 'uppercase', padding: '8px 18px', borderRadius: 30 }}>
                  Portal de Productores · Movies × Brands
                </div>
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 18, marginBottom: 28 }}>
              {statCard('🎬', 'Proyectos en pipeline', pipeline.length)}
              {statCard('💬', 'Mensajes', messages.length)}
              {statCard('🌍', 'Ecosistema', 'Activo')}
            </div>
            ...
            <div style={{ ...panel, borderColor: G }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                {[
                  ['Nombre', producer?.contact_name],
                  ['Email', producer?.email ?? user.email],
                  ['Empresa', producer?.company_name],
                  ['Teléfono', producer?.phone],
                  ['País', producer?.country],
                  ['Web', producer?.website],
                ].map(([label, value]) => value ? (
                  <div key={label as string}>
                    <div style={{ ...lbl }}>{label as string}</div>
                    <div style={{ fontSize: 14, color: '#fff' }}>{value as string}</div>
                  </div>
                ) : null)}
              </div>
              {producer?.bio && (
                <div style={{ marginTop: 18, paddingTop: 18, borderTop: `1px solid ${LINE}` }}>
                  ...
                  <p style={{ fontSize: 14, opacity: .8, lineHeight: 1.7, margin: 0 }}>{producer.bio}</p>
                </div>
              )}
            </div>
          </div>
        ...
        {tab === 'myfilms' && (
          <div>
            <h2 style={{ fontWeight: 300, fontSize: 32, color: G, marginBottom: 8 }}>Mis Proyectos</h2>
            <p style={{ opacity: .55, fontSize: 15, marginBottom: 28 }}>Proyectos de tu productora en el ecosistema Movies × Brands</p>

            {pipeline.length === 0 ? (
              <div style={{ ...panel, textAlign: 'center', padding: 60, opacity: .5 }}>
                <div style={{ fontSize: 32, marginBottom: 12 }}>🎬</div>
                <p style={{ fontSize: 15 }}>Todavía no tienes proyectos en el pipeline. Contáctanos para añadir tu próximo proyecto.</p>
                <button onClick={() => setTab('messages')} style={{ marginTop: 16, padding: '10px 24px', background: G, color: '#0b0b0b', border: 'none', borderRadius: 6, cursor: 'pointer', fontFamily: 'Georgia,serif', fontSize: 10, letterSpacing: '.28em', textTransform: 'uppercase', fontWeight: 600 }}>Contactar →</button>
              </div>
            ) : (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(300px,1fr))', gap: 20 }}>
                {pipeline.map(f => <FilmCard key={f.id} f={f} />)}
              </div>
            )}
          </div>
        )}
        ...
        {tab === 'pipeline' && (
          <div>
            <h2 style={{ fontWeight: 300, fontSize: 32, color: G, marginBottom: 8 }}>Pipeline MXB</h2>
            <p style={{ opacity: .55, fontSize: 15, marginBottom: 28 }}>Todos los proyectos activos en el ecosistema Movies × Brands</p>
            {pipeline.length === 0 ? (
              <div style={{ ...panel, textAlign: 'center', padding: 60, opacity: .5 }}>
                <div style={{ fontSize: 32, marginBottom: 12 }}>🎬</div>
                <p>No hay proyectos en el pipeline todavía.</p>
              </div>
            ) : (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(300px,1fr))', gap: 20 }}>
                {pipeline.map(f => <FilmCard key={f.id} f={f} />)}
...
function FilmCard({ f }: { f: PipelineFilm }) {
  const G = '#D4AF37'
  const CH = '#E8D5C4'
  const PANEL = '#161616'
  const LINE = '#2c2c2c'
  ...
  return (
    <div style={{ background: PANEL, border: `1px solid ${LINE}`, borderRadius: 10, padding: 22, display: 'flex', flexDirection: 'column', gap: 10 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <span style={{ fontSize: 9, letterSpacing: '.25em', textTransform: 'uppercase', color: f.status === 'open' ? '#6fcf97' : CH, border: `1px solid ${f.status === 'open' ? '#6fcf97' : LINE}`, padding: '3px 8px', borderRadius: 20 }}>{f.status === 'open' ? 'Abierto' : f.status === 'closed' ? 'Cerrado' : f.status}</span>
        {f.festival && <span style={{ fontSize: 11, opacity: .5 }}>🏆 {f.festival}</span>}
      </div>
      <h3 style={{ fontWeight: 300, fontSize: 19, color: G, margin: 0 }}>{f.title}</h3>
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
        {f.genre && <span style={{ fontSize: 11, opacity: .6 }}>🎭 {f.genre}</span>}
        {f.country && <span style={{ fontSize: 11, opacity: .6 }}>🌍 {f.country}</span>}
      </div>
      {f.synopsis && <p style={{ fontSize: 13, opacity: .65, lineHeight: 1.65, margin: 0 }}>{f.synopsis.slice(0, 140)}{f.synopsis.length > 140 ? '…' : ''}</p>}
      {f.funding_total && (
        <div>
          ...
          <div style={{ fontSize: 11, opacity: .5, marginTop: 5 }}>
            {f.funding_remaining ? `${f.funding_remaining.toLocaleString('es-ES')} € restantes de ${f.funding_total.toLocaleString('es-ES')} €` : `${f.funding_total.toLocaleString('es-ES')} € total`}
          </div>
        </div>
      )}
      {f.dossier_url && (
        <a href={f.dossier_url} target="_blank" rel="noopener noreferrer" style={{ fontSize: 11, color: G, textDecoration: 'none', letterSpacing: '.15em' }}>📄 Ver dossier →</a>
      )}
    </div>
  )
}
